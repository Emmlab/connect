...
const CommentCard = ({ comment }: { comment: PostCommentType }) => {
  ...
  const { mutate: mutateDeletePostComment, isPending: isPendingDeletePostComment } = useMutation({
        ...
        <CardTitle className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              <div className="line-clamp-1 text-base">{comment.developerName || "Unknown"}</div>
              <div className="text-xs italic">{format(comment.$createdAt, "LLL dd, y")}</div>
            </div>
          </div>
          <div className='flex items-center gap-2 my-2'>
            <CustomButton
              ...
              handleClick={() => comment && mutateDeletePostComment(comment.$id as string)}
