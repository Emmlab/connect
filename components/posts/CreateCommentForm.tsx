...
import { postCommentFormSchema, PostCommentFormType } from '@/utils/types/postLikesComments';
...
const CreateCommentForm = ({ postId }: { postId:string; }) => {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (values: PostCommentFormType) => createPostCommentAction({comment: values.comment, postId}),
    onSuccess: (data) => {
      if (!data) {
        toast({
