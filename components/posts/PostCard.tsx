...
import { deletePostAction, postLikeAction, postDisLikeAction } from '@/utils/actions/';
...
const PostCard = ({ post }: { post: PostType }) => {
  ...
  const { mutate: mutateLikePost, isPending: isPendingLikePost } = useMutation({
    mutationFn: (id: string) => postLikeAction({ postId: id, isLiked: post.liked, isDisLiked: post.disLiked, likesCount: post.likesCount, disLikesCount: post.disLikesCount }),
    onSuccess: (data) => {
      if (!data) {
        toast({
  ...
  const { mutate: mutateDisLikePost, isPending: isPendingDisLikePost } = useMutation({
    mutationFn: (id: string) => postDisLikeAction({ postId: id, isLiked: post.liked, isDisLiked: post.disLiked, likesCount: post.likesCount, disLikesCount: post.disLikesCount }),
    onSuccess: (data) => {
      if (!data) {
        toast({
          description: 'Something went wrong',
        ...
        <CardTitle className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              <div className="line-clamp-1 text-base">{post.developerName || "Anonymous"}</div>
              <div className="text-sm font-normal">
                Posted on <i className="text-xs">{format(post.$createdAt, "LLL dd, y")}</i>
              </div>
            </div>
          </div>
      ...
      <CardContent className=''>
        ...
        <div className='flex items-center gap-2 my-2'>
          ...
          {!isPendingDisLikePost ? (
            <CustomButton
            icon={<ThumbsUp className={`${post.liked ? "text-red-500" : ""}`} />}
          ...
          {!isPendingLikePost ? (
            <CustomButton
            icon={<ThumbsDown className={`${post.disLiked ? "text-red-500" : ""}`} />}
