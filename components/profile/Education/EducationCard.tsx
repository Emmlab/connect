...
const EducationCard = ({ educationItem }: { educationItem: EducationType }) => {
  ...
  const { mutate: mutateDeleteEducation, isPending: isPendingDeleteEducation } = useMutation({
      ...
      <CardHeader className="flex gap-2 p-2 md:px-5">
        <CardTitle className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            <div className="line-clamp-1 text-base">{educationItem.school || "Unknown"}</div>
            <div className="text-xs font-normal italic">{format(educationItem.startDate, "LLL dd, y")} - {format(educationItem.endDate, "LLL dd, y")}</div>
          </div>
        </CardTitle>
      </CardHeader>
        ...
        {!developerId ? (
          <div className='flex items-center gap-2 my-2 md:pl-[48px]'>
            <CustomButton
              ...
              text=""
              handleClick={() => router.push(`/profile/education/${educationItem.$id}`)}
              isPending={false}
              className="h-fit w-fit px-2 py-1" />
            <CustomButton
              icon={<Trash2 className="h-[18px]" />}
              text=""
              handleClick={() => educationItem && mutateDeleteEducation(educationItem.$id)}
