...
const WorkExperienceCard = ({ workExperienceItem }: { workExperienceItem: WorkExperienceType }) => {
  ...
  const { mutate: mutateDeleteWorkExperience, isPending: isPendingDeleteWorkExperience } = useMutation({
      ...
      <CardHeader className="flex gap-2 p-2 md:px-5">
        <CardTitle className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            <div className="line-clamp-1 text-base">{workExperienceItem.company || "Unknown"}</div>
            <div className="text-sm font-normal">
              {workExperienceItem.role} - <i className="text-xs">{format(workExperienceItem.startDate, "LLL dd, y")} - {format(workExperienceItem.endDate, "LLL dd, y")}</i>
            </div>
          </div>
        </CardTitle>
      </CardHeader>
        ...
        {!developerId ? (
          <div className='flex items-center gap-2 my-2 md:pl-[48px]'>
            ...
            <CustomButton
              ...
              text=""
              handleClick={() => router.push(`/profile/work-experience/${workExperienceItem.$id}`)}
              isPending={false}
            ...
            <CustomButton
              ...
              text=""
              handleClick={() => mutateDeleteWorkExperience(workExperienceItem.$id)}
